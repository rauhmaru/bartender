...
## 2024-08-01 - [Global CSRF Protection]
**Vulnerability:** The application was lacking global Cross-Site Request Forgery (CSRF) protection on forms, leaving endpoints that mutate state (insert, update, delete) vulnerable.
**Learning:** Using Flask-WTF (`CSRFProtect(app)`) provides an easy and robust way to enforce CSRF validation on all POST/PUT/DELETE requests globally across the application. When this is introduced, every form must be updated to include the token, otherwise submissions will fail with a 400 error.
**Prevention:** Always initialize `CSRFProtect(app)` in Flask applications early in the setup and ensure that all Jinja2 templates include `<input type="hidden" name="csrf_token" value="{{ csrf_token() }}"/>` in state-mutating forms.

...
## 2026-06-30 - [Missing CSRF Protection]
**Vulnerability:** The application was missing Cross-Site Request Forgery (CSRF) protection on forms, which could allow attackers to perform actions on behalf of authenticated users without their consent.
**Learning:** Adding CSRF protection globally in a Flask application requires setting up `flask-wtf` and explicitly rendering `<input type="hidden" name="csrf_token" value="{{ csrf_token() }}"/>` in all forms with `method="post"`. Additionally, when using Pytest to test endpoints, you must disable CSRF in test client configs (`app.config['WTF_CSRF_ENABLED'] = False`) to prevent artificial failures.
**Prevention:** Always use `Flask-WTF`'s `CSRFProtect(app)` globally, and review all templates to verify the CSRF hidden field is present in any POST form. Document the requirement for disabling it in tests.

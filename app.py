...
def inserir_tipo(nome):
    ...
    with db_lock:
        # ⚡ Bolt: Use contains() to short-circuit evaluation and avoid full table scan
...
def visualizar_cocktails():
    ...
    cocktails_db = cocktails_table.all()
    if selecionados:
        # ⚡ Bolt: Filter raw db entries before mapping to avoid O(N) operations on discarded items
        cocktails_db = [
            c

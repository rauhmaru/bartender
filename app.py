...
def inserir_tipo(nome):
    ...
    with db_lock:
        # ⚡ Bolt: use .contains() instead of .search() for early return and better performance
        existing = tipos_table.contains(Tipo.nome == nome)
        if existing:
...
def atualizar_tipo(tipo_id, nome):
    ...
    old_nome = old["nome"]
    # ⚡ Bolt: use .contains() instead of .search() to short-circuit lookup
    existing = tipos_table.contains((Tipo.nome == nome) & (Tipo.id != tipo_id))
    if existing:
        raise ValueError(f"O tipo '{nome}' já está cadastrado.")
...
def tipo_em_uso(tipo_id):
    ...
    if not tipo_doc:
        return False
    nome = tipo_doc["nome"]
    # ⚡ Bolt: use .contains() instead of full search/len() scan
